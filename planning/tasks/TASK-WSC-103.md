...
evidence: planning/tasks/TESTRUN-TASK-WSC-103.md
goal: |
  空间/行业/子类维护弹窗与 API；待保存提示；有挂载禁止删（DEC-WSC-003）；至少保留一个三级。
reqs:
  - REQ-CAT-006
dependsOn:
  - TASK-WSC-102
...
mergeConstraint: 禁止与 TASK-WSC-104 混写同一 PR
```

## 验收要点

见 `planning/approved/PLAN-WSC-2.2.md` § TASK-WSC-103。

- 三级分栏增改；级联刷新；非管理员不可达
- 删挂载分类返回明确错误码；至少保留一个三级
