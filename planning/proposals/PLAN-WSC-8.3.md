# 接入端工作台（WSC）V1.6 候选执行计划 — 目录筛选 / 座序图 L1 / 我的目录 / 企业 scope（Round 3）
...
actorInstance: plan-editor-wsc-011-r3
```

> **候选声明**：本文件为 `PLAN-WSC-8.3` **Round 3** 候选稿（`planType: CANDIDATE` / `status: DRAFT`），基于 `PLAN-WSC-8.2` Round 2 评审修订。须经规划委员会**隔离**独立复评后方可进入 `APPROVED`；本 planEditor 实例**不**伪造任何委员会批准，**不**自行关闭任何 ISSUE，**不**将 `status` 标为 `APPROVED`，**不**写入 `ai/runs/**/state.yaml` 或 `events.jsonl`。  
...
> 下列映射吸收 Round 1 / Round 2 各 REV 的 `closeWhen` 修订意图；**ISSUE 状态未关闭**，须由原提出者在 Round 3 独立复评后确认。
...
| ISSUE-PE-WSC8-R2-001 | planEditor | §0.3、§5 907 denyModify | 907 `denyModify` 删除错误 `frontend/src/common/security/WriteAuthorizationInterceptor.java`；改为与 905 `writeSet` 一致的 backend 字面路径 `backend/app/data-chain-service/src/main/java/com/shdata/datachain/common/security/WriteAuthorizationInterceptor.java`；§0.3 同步全路径 |
...
| 概念 | 本计划取值 | 含义 |
|---|---|---|
| `planId` | PLAN-WSC-8.3 | V1.6 Round 3 候选执行计划 |
| `round` | 3 | 本 planId 共识轮次（Round 3 独立复评） |
| `basedOn` | PLAN-WSC-8.2 | Round 2 候选 + R2 评审修订 |
...
| `CatalogBrowseController` / `CatalogBrowseService` / `RbacMatrix` / `backend/.../common/security/WriteAuthorizationInterceptor.java`（scope 面） | **905** | 902 可写 `L2Distribution*`；904 不得改 browse list |
...
| maintenance controller/service（scope API） | **907**（BE scope 实现） | 905 独占 interceptor/matrix；907 **不得**改 `backend/app/data-chain-service/src/main/java/com/shdata/datachain/common/security/WriteAuthorizationInterceptor.java` |
...
| Blocking OQ | **无** | 可送委员会 Round 3 复评 |
...
- `readSet`:
  ...
  - `planning/proposals/PLAN-WSC-8.3.md`（本文件 §1/§0.3）
...
- `denyModify`:
  ...
  - `frontend/src/features/auth/composables/useCanWrite.ts`
  - `frontend/src/features/catalog/browse/**`
  - `backend/app/data-chain-service/src/main/java/com/shdata/datachain/common/security/WriteAuthorizationInterceptor.java`（归 905）
  - `contracts/**`；`frontend/src/api/**`
  - `L2Distribution*`
...
- [x] 引用 SNAP-WSC-008 / RUN-WSC-011；`planId: PLAN-WSC-8.3`；`status: DRAFT`；`planType: CANDIDATE`；`round: 3`；`basedOn: PLAN-WSC-8.2`
- [x] §「修订说明 / 待 R3 复评」映射 R1/R2 ISSUE（**未关闭**）
- [x] 907 denyModify `WriteAuthorizationInterceptor` backend 字面路径与 905 writeSet 对齐（ISSUE-PE-WSC8-R2-001 吸收，**未关闭**）
...
- [ ] 对本 planId 的规划委员会 **Round 3** 独立评审 / APPROVE（**尚未发生**；本文件不伪造）

---

**planEditor decision（本实例）**：`PLAN-WSC-8.3` 已落盘为 Round 3 **CANDIDATE / DRAFT**，吸收 PLAN-WSC-8.2 Round 2 评审意图（修正 907 denyModify 拦截器 backend 字面路径），**可以提交规划委员会 Round 3 隔离复评**。本实例 **不** 输出 APPROVE，**不** 关闭 ISSUE，**不** 写入 `ai/runs` state/events，**不** commit，**不** 写入 `planning/approved/`。
