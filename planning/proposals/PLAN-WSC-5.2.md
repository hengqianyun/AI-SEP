...
> **候选声明**：本文件为 `PLAN-WSC-5.2` **Round 2** 候选稿（`planType: CANDIDATE` / `status: DRAFT`），全文修订自 `PLAN-WSC-5.1`，吸收其 Round 1 隔离评审 ISSUE 的 closeWhen **意图**。须经规划委员会**隔离**独立评审后方可进入 `APPROVED`；本 planEditor 实例（`plan-editor-wsc-008-r2`）**不**伪造任何委员会批准，**不**自行关闭任何 ISSUE，**不**将 `status` 标为 `APPROVED`。  
