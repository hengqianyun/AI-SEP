# 接入端工作台（WSC）V1.7 候选执行计划 — 交易订单（链内一事一议简易流程）（Round 2）
...
|---|---|---|---|
| ISSUE-API-001 | apiDataDesigner | §3.1 / 913 / 916 | ChainAttestationPort 冻结为**并列方案 B**：新建 `OrderChainAttestationPort.attestOrder(OrderAttestationRequest(orderId, status, actorRole, actorUserId, snapshotJson))`，与现有 `ChainAttestationPort` 共存；913/916 注入对应端口；§3.1 表格冻结请求类型 + payload 字段枚举 |
| ISSUE-API-002 | apiDataDesigner | §3.1 / 916 | `POST /orders/{orderId}/contract` multipart **硬冻结** part 名 `file`（签署附件）+ `transactionInfo`（JSON string，含 `orderLines[{unit, quantity, unitPrice, subtotal}]`、`amountTaxInclusive`、`amountTaxExclusive`、`currency`）；明确 orderLines **仅**在 contract 阶段提交 |
| ISSUE-API-003 | apiDataDesigner | §3.1 / 913 | `GET /orders` 200 响应**枚举**九组字段 + 分页 envelope `{ total, list, page, pageSize }`；911 OpenAPI 写死 response schema |
| ISSUE-API-004 | apiDataDesigner | §3.1 / 916 | `POST /orders/{orderId}/confirm` 明确为**纯动作端点**，无 request body（空 JSON 对象） |
| ISSUE-UX-001 | uxUiPlanner | 917 | 统一合约确认页**布局冻结**（交易信息→须知→附件纵向分区）；未勾选→确认按钮 disabled；ADMIN 代确认页内可见操作人身份标识；testScope 增 Vitest disabled 断言 |
| ISSUE-UX-002 | uxUiPlanner | 917 | 附件上传交互**冻结**：上传中 Ant Upload progress；前端即时校验（格式/大小）；扫描失败 inline 错误；已上传文件显示文件名+大小+可删除/替换；testScope 增文件校验 Vitest |
| ISSUE-UX-003 | uxUiPlanner | 917 | 取消对话框**冻结**：标题「确认取消订单」；正文含「此操作不可回退」；确认按钮「确认取消」+ danger 样式；ADMIN 代取消体现代操作身份；testScope 增对话框断言 |
| ISSUE-SEC-001 | securityOperations | §1.2 / 913 | 订单号生成规则**指定**：UUID v4（不可预测、不含可推断业务语义字段）；913 acceptance 增订单号格式负例 |
| ISSUE-SEC-002 | securityOperations | 913 / 916 / §3.1 | 913 acceptance 增 `userId`/`demandUserId` 参数 IDOR 负例；916 acceptance 增确认/取消篡改 enterprise/user 参数负例；§3.1 OpenAPI 增安全说明 |
| ISSUE-SEC-003 | securityOperations | 916 / 913 | 916 acceptance 增 ADMIN 代操作审计条款（`update_by`=ADMIN 用户标识、时间线 operator 记录 ADMIN 身份）；913 确认 `create_by`=创建会话 USER |
| ISSUE-QA-001 | qaStrategist | 919 / §6.1 | 919 `requirements` 增 `REQ-WSC-ORDER-017`；§6.1 增场景 #10「数字合约版本」（建议）；918 testScope 增 Vitest 版本区 UI 断言 |
| ISSUE-SEC-004 | securityOperations | 913 / 916 | mock 上链实现**禁止**读取真实链节点配置（URL/私钥/证书）；mock 模式下链配置项应有安全默认值 |
| ISSUE-QA-002 | qaStrategist | 916 | 916 testScope 增「每步状态变更后 `chainCount` = 时间线记录条数；列表 chainCount 与详情一致」 |
| ISSUE-QA-003 | qaStrategist | 918 | 918 testScope「无 URL 提示」改为正向断言：「无地址时展示"未配置"或等价提示文案（testid 可断言）」 |
| ISSUE-API-005 | apiDataDesigner | §3.1 | `GET /orders/notices/current` 响应含 `{ content, version }`；`notice_version` 由服务端自动填充 |
| ISSUE-API-006 | apiDataDesigner | YAML 元数据 | YAML 头 `contractsTarget: wsc-contracts@2.3.2` 保持（与 SNAP 一致）；§0.1 补充说明：工作树当前为 2.3.3，由 911 唯一管理增量 |

---

> **候选声明**：本文件为 `PLAN-WSC-9.2` **Round 2** 候选稿（`planType: CANDIDATE` / `status: DRAFT`），基于 `PLAN-WSC-9.1` Round 1 评审修订。须经规划委员会**隔离**独立复评后方可进入 `APPROVED`；本 planEditor 实例**不**伪造任何委员会批准，**不**自行关闭任何 ISSUE，**不**将 `status` 标为 `APPROVED`，**不**写入 `REV-*`，**不**写入 `ai/runs/**/state.yaml` 或 `events.jsonl`。
>
...
| 概念 | 本计划取值 | 含义 |
|---|---|---|
| `planId` | PLAN-WSC-9.2 | V1.7 Round 2 候选执行计划 |
| `round` | 2 | 本 planId 共识轮次 |
| `basedOn` / `lineageFrom` | PLAN-WSC-9.1 / PLAN-WSC-8.3 | 相对 Round 1 的修订增量；**不抄** 8.3 业务任务（901–908） |
...
| 契约目标 | **`wsc-contracts@2.3.2`**（V1.7 **增量**挂在当前工作树已发布 semver 上）<!-- R2 revised: ISSUE-API-006 --> | 由 **TASK-WSC-911** **唯一**拥有契约/doc 增量与 `frontend/src/api/**` 生成；**禁止**他任务 bump `VERSION`。注：工作树当前 `VERSION` = **2.3.3**（高于 SNAP 声明的 2.3.2），四头已同步为 2.3.3；911 在此之上合并 V1.7 订单增量，**默认不再升 semver** |
...
| 订单号生成 | <!-- R2 revised: ISSUE-SEC-001 --> **UUID v4**（不可预测、不含可推断业务语义的字段如企业 ID/产品编码/用户 ID）；不同订单的订单号无可观察递增规律 |
...
| 平台统一须知 | 服务端只读资源 + 勾选令牌；每次链内提交须勾选；正文可查看；响应含版本标识（§3.1） |
| 状态机 | 待确认订单 → 待上传合约 → 待确认合约 → 合约已达成；旁路已取消；**不可回退**；无拒绝/退回/超时 |
| 上链 | **mock**；创建及每次状态变更写时间线 + mock 记录；列表 `chainCount` = 详情记录数 |
| mock 上链隔离 | <!-- R2 revised: ISSUE-SEC-004 --> mock 上链实现**禁止**读取/暴露真实链节点配置（URL/私钥/证书）；mock 模式下链相关配置项应有安全默认值（如空字符串或 localhost 占位）；mock 记录的哈希/高度/节点字段为模拟值，不匹配真实链浏览器可验证格式 |
...
<!-- R2 revised: ISSUE-API-001, ISSUE-API-002, ISSUE-API-003, ISSUE-API-004, ISSUE-API-005, ISSUE-SEC-002, ISSUE-API-006 -->
...
| `GET /orders` 200 响应 | <!-- R2 revised: ISSUE-API-003 --> 九组字段枚举：①订单号（`orderId`）②产品信息（`productName`、`productCode`、`productType`、`sourcePlatform`）③提供方（`providerEnterpriseName`）④需求方（`demandUserName`、`demandUserLoginName`、`demandEnterpriseName`）⑤金额（`amountTaxInclusive`、`amountTaxExclusive`、`currency`）⑥状态（`status`、`statusLabel`）⑦上链次数（`chainCount`）⑧下单时间（`createTime`）⑨操作（由 FE 按角色渲染）。分页 envelope：`{ total: number, list: OrderListItem[], page: number, pageSize: number }` |
...
| `POST /orders/{orderId}/confirm` | <!-- R2 revised: ISSUE-API-004 --> 确认订单；**纯动作端点，无 request body（空 JSON 对象）**；PROVIDER 本企业或 ADMIN；`PENDING_CONFIRM` → `PENDING_UPLOAD` |
| `POST /orders/{orderId}/contract` | <!-- R2 revised: ISSUE-API-002 --> multipart **硬冻结**：part `file`（签署附件，required，Word/PDF ≤20MB）+ part `transactionInfo`（JSON string，required）；`transactionInfo` schema：`{ orderLines: [{ unit: string, quantity: number, unitPrice: string, subtotal: string }], amountTaxInclusive: string, amountTaxExclusive: string, currency: "CNY" }`。仅需求方本人；`PENDING_UPLOAD` → `PENDING_CONTRACT_CONFIRM`；无合格附件 4xx；`orderLines` **仅**在 contract 阶段提交，`POST /orders` 创建时不含计费 |
...
| `GET /orders/notices/current` | <!-- R2 revised: ISSUE-API-005 --> 平台统一须知正文；响应 `{ content: string, version: string }`；`notice_version` 由服务端自动填充至 `t_order_header`，前端无需回传版本号 |
...
| 安全说明 | <!-- R2 revised: ISSUE-SEC-002 --> 订单 scope **仅**信 SessionPrincipal；拒绝客户端 enterprise/user 作授权；授权范围**仅**从服务端会话推导，客户端 `enterpriseId`/`userId`/`demandUserId` 参数**不**作为授权依据（OpenAPI description 写死） |
...
| `ChainAttestationPort.md` | <!-- R2 revised: ISSUE-API-001 --> **并列方案**：新建 `OrderChainAttestationPort` 接口（`attestOrder(OrderAttestationRequest)`），与现有 `ChainAttestationPort`（面向产品目录快照）共存。`OrderAttestationRequest` 字段：`orderId`（string）、`status`（string，状态枚举值）、`actorRole`（string，ADMIN/PROVIDER/USER）、`actorUserId`（string）、`snapshotJson`（string，订单快照摘要）。913/916 注入 `OrderChainAttestationPort`；mock 适配器返回模拟哈希/高度/时间；**不要求**真链 |
...
- 订单号：<!-- R2 revised: ISSUE-SEC-001 --> `t_order_header.order_no` 字段类型 `VARCHAR(36) NOT NULL`，存储 UUID v4 格式；生成逻辑在 service 层，**不含**企业 ID/产品编码/用户 ID 等可推断业务语义的字段。
- 建议表（可在同版本脚本内，主题仍为订单）：
  - `t_order_header`：订单号、状态、需求方用户、提供方企业、产品 id/编码、产品快照 JSON、备注、须知版本、含税金额、未税金额、币种、上链次数缓存等
  - `t_order_line`：计费明细（单位、数量、单价、小计）；**仅**由 `POST /orders/{orderId}/contract` 写入
  - `t_order_event`：时间线
  - `t_order_chain_log`：mock 上链展示字段（哈希、高度、时间等，来源模拟适配器）；<!-- R2 revised: ISSUE-SEC-004 --> mock 实现**禁止**写入真实链节点 URL/私钥/证书
...
- `readSet`:
  ...
  - `planning/proposals/PLAN-WSC-9.2.md`（本文件 §3 / §4）
  - 既有 `contracts/**`（含 VERSION 2.3.3、V1.6 matrix）
- `writeSet`:
  - `contracts/**`（含 `VERSION`、`openapi/**`、`rbac/matrix.yaml`、`ui/state-matrix.md`、`req-coverage.md`、`errors/codes.yaml`、`chain/ChainAttestationPort.md`、`chain/OrderChainAttestationPort.md`、`security/sensitive-fields.md` 若需订单补充）
  - `frontend/src/api/**`
...
- `readSet`:
  ...
  - 既有 `SessionPrincipal`、`RbacMatrix`、`WriteAuthorizationInterceptor`
...
- `writeSet`:
  ...
  - `backend/app/data-chain-service/src/main/java/com/shdata/datachain/common/port/OrderChainAttestationPort.java`（接口定义 + `SimulatedOrderChainAttestationAdapter` mock 实现）
...
| 10 | 数字合约版本 | <!-- R2 revised: ISSUE-QA-001 --> 创建后版本 1 可见；主路径达成后版本 ≥2 且历史可查 | **建议** |
...
| REQ-WSC-ORDER-017 | P1 | 新增 | **918** + **919** | <!-- R2 revised: ISSUE-QA-001 --> 数字合约版本；919 E2E 建议场景 #10 |
...
| mock 上链暴露真实链配置 | <!-- R2 revised: ISSUE-SEC-004 --> §1.2 硬性禁止；mock 适配器安全默认值 | 移除真实链配置引用 |
...
| 订单号可预测/信息泄露 | <!-- R2 revised: ISSUE-SEC-001 --> §1.2 指定 UUID v4；913 负例断言 | 热修生成逻辑 |
...
- [x] 引用 SNAP-WSC-009 / RUN-WSC-012；`planId: PLAN-WSC-9.2`；`status: DRAFT`；`planType: CANDIDATE`；`round: 2`；`basedOn: PLAN-WSC-9.1`
- [x] §「修订说明 / 待 R2 复评」映射 R1 全部 16 条 ISSUE（**未关闭**，须原提出者 R2 复评确认）
...
- [x] R2 修订吸收：ChainAttestationPort 并列方案（ISSUE-API-001）；multipart 字段名/schema 冻结（ISSUE-API-002）；列表九组字段+envelope 枚举（ISSUE-API-003）；confirm 无 body（ISSUE-API-004）；须知含版本（ISSUE-API-005）；contractsTarget 说明（ISSUE-API-006）；确认页布局+disabled+代操作标识（ISSUE-UX-001）；附件上传交互（ISSUE-UX-002）；取消对话框（ISSUE-UX-003）；UUID v4 订单号（ISSUE-SEC-001）；IDOR 负例（ISSUE-SEC-002）；ADMIN 代操作审计（ISSUE-SEC-003）；mock 上链隔离（ISSUE-SEC-004）；数字合约版本 E2E+UI（ISSUE-QA-001）；chainCount 逐态（ISSUE-QA-002）；外部跳转正向断言（ISSUE-QA-003）
- [x] 对本 planId 的规划委员会 **Round 2** 独立评审 / **全员 APPROVE**（2026-08-20）

---

**planEditor decision（本实例）**：`PLAN-WSC-9.2` 已落盘为 Round 2 **CANDIDATE / DRAFT**，吸收 `PLAN-WSC-9.1` Round 1 全部 16 条 ISSUE（1×P0 + 11×P1 + 4×P2）的修订意图。**ISSUE 状态均未关闭**，须由原提出者在 Round 2 独立复评后确认。本实例 **不** 输出 APPROVE，**不** 关闭 ISSUE，**不** 写入 `ai/runs` state/events，**不** commit，**不** 写入 `planning/approved/`。
