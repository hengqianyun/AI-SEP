# UI 状态矩阵引用（wsc-contracts@2.0.0）

> 权威来源：PLAN-WSC-1.1 §3.5 + PLAN-WSC-2.2 §3.6（导入四态）。
...
| ④ 文件级拒绝 | 请求级错误（超限/非法格式） | 停留上传步；映射 `ERR_IMPORT_FILE_TOO_LARGE` 等；不伪装成行级结果态 |
