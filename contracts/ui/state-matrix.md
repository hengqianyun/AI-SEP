# UI 状态矩阵引用（wsc-contracts@2.2.0）

> 权威来源：PLAN-WSC-1.1 §3.5 + PLAN-WSC-2.2 §3.6 + PLAN-WSC-4.1 §3.4（导入四态）+ PLAN-WSC-5.2 §1.5 / §3.1（V1.4 壳层）。
...
| 我的数据产品列表 | 列表 loading | 无本人产品空态 | 加载失败提示 | — |
...
## 侧栏可见性（V1.4 / SNAP-WSC-005）

| 菜单 | ADMIN | PROVIDER | USER |
|---|---|---|---|
| 数据目录（公共，只读 + 座序图） | 可见 | 可见 | 可见 |
| 我的数据产品（写入口：增改导） | 隐藏 | 可见 | 隐藏 |
| 目录维护 | 可见 | 隐藏 | 隐藏 |
| 用户管理 | 可见 | 隐藏 | 隐藏 |
| 角色区 | 只读标签 | 只读标签 | 只读标签 |

## 导入结果四态（PLAN-WSC-2.2 §3.6 / PLAN-WSC-4.1 §3.4；宿主迁至我的产品）
