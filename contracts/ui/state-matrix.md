...
> 权威来源：PLAN-WSC-8.3 §3.1 + SNAP-WSC-008 V1.6 导航/RBAC + SNAP-WSC-009 V1.7 订单。
...
| 订单列表（`/orders`） | 列表 loading | 暂无订单数据 | 加载失败提示 | — |
| 订单详情（`/orders/:orderId`） | 详情 loading | — | 加载失败/越权 403 | — |
| 订购入口（`/orders/subscribe/:productId`） | 加载中 | — | 加载失败 | 创建成功 toast |
| 统一合约确认页 | 提交中禁用按钮 | — | 校验/业务错误码映射提示 | 确认成功 toast |
| 取消订单（二次确认） | — | — | — | 取消成功 toast |

## 侧栏可见性（V1.6 / SNAP-WSC-008 + V1.7 / SNAP-WSC-009）
...
| 分类维护 | 可见 | 隐藏 | 隐藏 |
| 用户管理 | 可见 | 隐藏 | 隐藏 |
| 交易订单（`/orders`；PROVIDER/ADMIN 文案） | 可见 | 可见 | **隐藏** |
| 我的订单（`/orders`；USER 文案） | **隐藏** | **隐藏** | 可见 |
| 角色区 | 只读标签 | 只读标签 | 只读标签 |
| 企业信息区（P1） | 只读 | 只读 | 只读 |

> ADMIN **双入口**：目录维护（全量）与我的目录（本企业）路由/active/页头可区分。PROVIDER **无**目录维护菜单与深链。
>
> **V1.7 订单菜单角色化**：USER 侧栏文案为「我的订单」；PROVIDER/ADMIN 侧栏文案为「交易订单」。两角色共用 `/orders` 路由，菜单文案按角色区分。
> 数据登记、连接器管理仍保持「本版本未开放」占位（不在 V1.7 范围内）。V1.6 目录 IA 不回退。
